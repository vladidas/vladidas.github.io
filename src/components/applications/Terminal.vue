...
<script>
    ...
    export default ({
        ...
        data() {
            return {
                ...
                commands: [
                    ...
                    { name: '/about', info: 'Show info about Vladidas.', call: this.callInfoCommand },
        ...
        methods: {
            ...
            writingText(text) {
                let lastCommandLine = this.$refs.terminal.querySelectorAll('.command:last-child')[0];
                lastCommandLine.innerHTML = `${text || ''}`;
            },
            ...
            init() {
                ...
                window.addEventListener('keyup', function(event) {
                    if (_this.$refs.terminal.closest('.window-border').classList.contains('active')) {
                        switch (event.key) {
                            case 'Backspace':
